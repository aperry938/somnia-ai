...
const CARD_TYPES: { id: AnalyticsCardType; label: string; description: string }[] = [
    { id: 'weekly-wrap', label: 'Weekly Wrap', description: 'Dreams, quality & themes' },
    { id: 'dream-journey', label: 'Dream Journey', description: 'Level & progress' },
    { id: 'mood-insights', label: 'Mood Insights', description: 'Emotional patterns' },
    { id: 'sleep-quality', label: 'Sleep Score', description: 'Quality rating' },
];
...
export const ShareAnalyticsModal: React.FC<ShareAnalyticsModalProps> = ({ dreams, isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        const generatePreview = async () => {
            ...
            try {
                const url = await generateAnalyticsCard({
                    dreams,
                    format,
                    theme,
                    cardType,
    ...
    const handleDownload = () => {
        ...
        haptics.medium();
        const filename = `somnia-${cardType}-${format}.png`;
    ...
    const handleShare = async () => {
        ...
        const title = `My ${CARD_TYPES.find(t => t.id === cardType)?.label} from Somnia`;
        ...
        >
            ...
            >
                ...
                <div className="p-4 space-y-5">
                    ...
                    {/* Card Type Selection */}
                    <div>
                        <label className="text-sm text-day-text-secondary dark:text-night-text-secondary mb-2 block">Card Type</label>
                        <div className="grid grid-cols-2 gap-2">
                            {CARD_TYPES.map((type) => (
                                <button
                                    key={type.id}
                                    onClick={() => { haptics.light(); setCardType(type.id); }}
                                    className={`p-3 rounded-xl border-2 transition-all text-left ${cardType === type.id
                                ...
                                >
                                    <p className="font-medium flex items-center gap-2">
                                        <span className="text-day-accent dark:text-night-accent">{CARD_TYPE_ICONS[type.id]}</span>
                                        {type.label}
                                    </p>
                                    <p className="text-xs text-day-text-secondary dark:text-night-text-secondary">{type.description}</p>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {FORMATS.map((f) => (
                                <button
                                    key={f.id}
                                    onClick={() => { haptics.light(); setFormat(f.id); }}
