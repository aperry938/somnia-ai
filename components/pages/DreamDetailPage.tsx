...
export const DreamDetailPage: React.FC<{ dreamId: number | null; onBack: () => void; }> = ({ dreamId, onBack }) => {
    ...
    const handleNarrate = useCallback(() => {
        ...
        let narrateText = dream.dreamText;

        // Add analysis if available
        if (dream.aiAnalysis) {
            narrateText += '. Analysis: ';
            dream.aiAnalysis.analysis.forEach(a => {
                narrateText += `${a.title}. ${a.content}. `;
            });
            narrateText += `Integration: ${dream.aiAnalysis.integration.content}`;
        }

        speakText(narrateText);
        setIsNarrating(true);
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className={`text-sm flex items-center gap-1 ${isNarrating ? 'text-day-accent dark:text-night-accent' : 'text-day-text-secondary dark:text-night-text-secondary hover:text-day-accent dark:hover:text-night-accent'}`}
                            title={isNarrating ? 'Stop narration' : 'Narrate dream'}
                        >
