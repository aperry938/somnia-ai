        ...
        <div className="bg-day-card-bg dark:bg-night-card-bg backdrop-blur-lg border border-day-border dark:border-night-border rounded-xl overflow-hidden">
            ...
            {isExpanded && (
                <div className="border-t border-day-border dark:border-night-border animate-fadeIn">
                    ...
                    {/* Manual Activities (retroactive logging) */}
                    {entry.sleepAids?.manualActivities && entry.sleepAids.manualActivities.length > 0 && (
                        <div className="px-4 py-2 bg-purple-50 dark:bg-purple-900/20 border-b border-day-border dark:border-night-border">
                            <p className="text-xs text-purple-600 dark:text-purple-400 font-medium mb-1.5">Pre-Sleep Activities</p>
                            <div className="flex flex-wrap gap-2">
                                {entry.sleepAids.manualActivities.map((activity, i) => (
                                    <span key={i} className="text-xs px-2 py-1 bg-purple-100 dark:bg-purple-800/50 text-purple-600 dark:text-purple-300 rounded-full flex items-center gap-1">
                                        {activity.name}
                                        {activity.duration && <span className="opacity-70">({activity.duration}m)</span>}
                                    </span>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* Wake Data (alarm metrics) */}
                    {entry.wakeData && (
                        <div className="px-4 py-2 bg-amber-50 dark:bg-amber-900/20 border-b border-day-border dark:border-night-border">
                            <p className="text-xs text-amber-600 dark:text-amber-400 font-medium mb-1.5">Wake Metrics</p>
                            <div className="flex flex-wrap gap-3 text-xs">
                                {entry.wakeData.snoozeCount !== undefined && entry.wakeData.snoozeCount > 0 && (
                                ...
                                {entry.wakeData.timeToSilence !== undefined && (
                                    <span className="flex items-center gap-1 text-amber-700 dark:text-amber-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                                        </svg>
                                        {Math.round(entry.wakeData.timeToSilence / 60)}m to silence
                                    </span>
                                )}
                                {entry.wakeData.didBoost && (
                                    <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                                        </svg>
                                        Alertness boost
                                        {entry.wakeData.boostDuration && <span className="opacity-70">({Math.round(entry.wakeData.boostDuration / 60)}m)</span>}
                                    </span>
                                )}
                                {entry.wakeData.alarmType && (
                    ...
                    {/* Manually Logged Indicator */}
                    {entry.manuallyLogged && (
                        <div className="px-4 py-1.5 bg-gray-50 dark:bg-gray-800/30 border-b border-day-border dark:border-night-border">
                            <span className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                                Activities logged retroactively
                            </span>
                        </div>
                    )}
